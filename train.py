...
"""
Every time you train this model, please carefully verify the following settings!

1. Are you training Tiny ImageNet or other datasets? (modify the way to obtain dataloader around line 490)
2. Are you expanding FC layers in transformer? (modify the code in transformer.py)
3. How many #layers, #heads, mlp_ratio, embedding_dim and kernel_size are you using? (modify the code in vit.py)
...
from src import *
from src.utils.dataloader_tiny_imagenet import TinyImageNet
from src.utils.activation import Learnable_Relu
from src.utils.KDLoss import KLLossSoft, KLTokenMSELoss
...
parser.add_argument('-c', '--config', default='/home/wenxuanzeng/projects/Compact-Transformers-main/configs/datasets/cifar10.yml', type=str, metavar='FILE',
                    help='YAML config file specifying default arguments')

parser = argparse.ArgumentParser(description='PyTorch ImageNet Training')

# Dataset / Model parameters
parser.add_argument('data_dir', metavar='DIR', default='/home/wenxuanzeng/data/cifar-10',
...
parser.add_argument('--model', default='vit_7_4_32', type=str, metavar='MODEL',
...
parser.add_argument('--no-prefetcher', action='store_true', default=True,
                    help='disable fast prefetcher')  # if you are training Tiny-ImageNet, set True (Origin: False)
...
# Ours: MPCViT
parser.add_argument('--lambda-lasso', default=1e-5, type=float, 
                    help='Lasso coefficient (default: 1e-5)')
...
parser.add_argument('--retrain-mode', action='store_true', default=False,
...
def main():
    ...
    args.distributed = False  # set whether distributed or not
    if 'WORLD_SIZE' in os.environ:
        args.distributed = int(os.environ['WORLD_SIZE']) > 1
    args.device = 'cuda:2,3'  # set gpu
    args.world_size = 1
    ...
    if args.distributed:
        if has_apex and use_amp != 'native':
            ...
            # if some parameters are not be used and do not return gradient, find them and ignore them!
    ...
    dataset_eval = create_dataset(
        name=args.dataset, root=args.data_dir, split=args.val_split, is_training=False, batch_size=args.batch_size)
    ...
    if 'imagenet' in args.data_dir:
        ...
        dataset_train = TinyImageNet(root=args.data_dir, train=True, transform=data_transforms['train'])
        dataset_eval = TinyImageNet(root=args.data_dir, train=False, transform=data_transforms['eval'])
    ...
    print('val_loss_fn:', validate_loss_fn)
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            print('Epoch costs time: {:.2f} s'.format(time_train_end - time_train_start))
        ...
        # re-define the CheckpointSaver!!! Otherwise, it won't save our new model!!!
        saver = CheckpointSaver(
            model=model, optimizer=optimizer, args=args, model_ema=model_ema, amp_scaler=loss_scaler,
            checkpoint_dir=output_dir, recovery_dir=output_dir, decreasing=decreasing, max_history=args.checkpoint_hist)
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = retrain_one_epoch(
                epoch, model, model_teacher, loader_train, optimizer, train_loss_fn, args,
                lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir,
                amp_autocast=amp_autocast, loss_scaler=loss_scaler, model_ema=model_ema, mixup_fn=mixup_fn)
...
def search_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if args.search_mode:
                # print('Searching: CELoss with Lasso term.')
                loss = loss_fn(output, target) + args.lambda_lasso * reg_loss  # add a Lasso term here
            else:
                # print('Baseline: CELoss only.')
...
def retrain_one_epoch(
        epoch, model, model_teacher, loader, optimizer, loss_fn, args,
        lr_scheduler=None, saver=None, output_dir=None, amp_autocast=suppress,
        loss_scaler=None, model_ema=None, mixup_fn=None):
    if args.mixup_off_epoch and epoch >= args.mixup_off_epoch:
        if args.prefetcher and loader.mixup_enabled:
